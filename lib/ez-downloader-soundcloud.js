...
class EzSoundcloudDownloader {
  ...
  _handleDownload(trackInfo, downloadTrackPromise) {
  ...
  async download(track_url) {
    let trackInfo = null, filename = null, coverUrl = null
    try {
      var response = await request(this.getTrackInfoURL(track_url))
      trackInfo = JSON.parse(response)
      filename = `${trackInfo.title}.${trackInfo.original_format}`
      coverUrl = trackInfo.artwork_url.replace(/large/gi, "t500x500")
      await this._handleDownload(trackInfo,
